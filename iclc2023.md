...
![Ziffers plugin for MuseScore exporting fragment from Aphex Twin's Avril 14th score imported from MIDI file.\label{mplugin}](images/musescore.png){width=90%}\
\begin{center}
\textbf{Figure 2:} \textit{Ziffers plugin for MuseScore exporting fragment from Aphex Twin's Avril 14th score imported from MIDI file.}
\end{center}

MuseScore plugin can be used to transform traditional scores (from MusicXML, ABC notation or MIDI files) to the numeric notation for analyzing the musical structures from your favorite composers and using the displaced sequences as form of rehersal and inspiration for live coding. Examples above and below exemplifies this form of learning practice using the extracted sequence from Avril 14th by Aphex Twin. 

~~~~ {.ruby}
ziffers " // Aphex Twin - Avril 14th in Ziffers notation with some example transformations that can be modified on-the-fly
/ synth: piano
...
![Pitches in integer notation.](images/pc_line.svg){width=90%}\
\begin{center}
\textbf{Figure 3:} \textit{Pitches in integer notation.}
\end{center}
...
![Pitches in degree notation.](images/degree_line.svg){width=85%}\
\begin{center}
\textbf{Figure 4:} \textit{Pitches in degree notation.}
\end{center}
